...
export function createGlobe(canvas, variant){
  ...
  function frame(now){
    ...
    // Target rotation
    if (mouse.inside && !loadingMode){
      rot.targetY = (mouse.x - 0.5) * Math.PI * preset.parallaxStrength;
      rot.targetX = preset.tilt + (mouse.y - 0.5) * 0.6;
    } else {
      rot.targetY += (loadingMode ? preset.idleSpin * 4 : preset.idleSpin) * dt;
      rot.targetX = preset.tilt;
    }
    // Ease current toward target
    rot.y += (rot.targetY - rot.y) * Math.min(1, dt * 1.5);
    rot.x += (rot.targetX - rot.x) * Math.min(1, dt * 1.5);
  ...
  return {
    ...
    dissolve,
    reset,
    startLoading(){ loadingMode = true; },
    stopLoading(){  loadingMode = false; },
    destroy(){
